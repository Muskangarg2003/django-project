...
def emp_login(request):
    ...
    if request.method =="POST":
        ...
        user=authenticate(username=u,password=p) #return user if password and email id match

        if user:
            login(request,user) #this is inbuilt login function that starts a session when user login
            error="no"
        else:
            error="yes"
...
def Logout(request):
    logout(request) #this is inbuilt login function that expire the session builkt after login
...
def change_password(request):
    ...
    if request.method == "POST":
        ...
        try:
            if user.check_password(c):#check_password and set_password is inbuilt function jo check krega ki user ne jo current password bhra hai vo shi hai ke nhi
                user.set_password(n) 
                user.save()
